import './protfolio.scss'
import ReactCardFlip from 'react-card-flip'
import { IconButton } from '@material-ui/core';
import { GitHub, YouTube } from '@material-ui/icons';
import { useState } from 'react';

const Protfolio = () => {
    const [isFlipped0,setIsFlipped0] = useState(false)
    const [isFlipped1,setIsFlipped1] = useState(false)
    const [isFlipped2,setIsFlipped2] = useState(false)
    const [isFlipped3,setIsFlipped3] = useState(false)


    const handleClick = (e) => {
        if(e.target.id === "0"){
            setIsFlipped0(!isFlipped0)
        }
        if(e.target.id === "1"){
            setIsFlipped1(!isFlipped1)
        }
        if(e.target.id === "2"){
            setIsFlipped2(!isFlipped2)
        }
        if(e.target.id === "3"){
            setIsFlipped3(!isFlipped3)
        }
        

    }
    return (
        <div className='protfolio' id='protfolio'>
            <h1>Portfolio</h1>
            
            <ul>
                <li className='active'>Web App</li>
            </ul>

            <div className="container">
               
                    
            

                <ReactCardFlip isFlipped={isFlipped0} flipDirection="horizontal">
                <div onClick={handleClick} > 
                
                <div className="item">
                        <img  id="0" src="data:image/jpeg;base64," alt="" />
                        <h3  id="0">Cinema App</h3>
                </div>
                       
                    </div>

                    <div id="0" onClick={handleClick} className="backItem">
                         <h3 id="0" className='backTop'>Cinema App</h3>
                         <span id="0" className="backText">
                        This is my first big project, I used 2 difftent apis one of them is reading data from a json file and implants it in the api both are connected 
                        to the DB. i used Mongo and for the whole app i used react and JWT for DB protection
                        
                        </span> 
                        <div className="iconButton">
                            
                        <IconButton>
                        <a href="https://github.com/deangal/cinema">
                        <GitHub/>
                        </a>
                        </IconButton> 

                        <IconButton>
                        <a href="https://www.youtube.com/watch?v=Y5MyAH9V_4w">
                        <YouTube/>
                        </a>
                        </IconButton> 
                        
                       
                        
                       
                        </div>
                </div>
                </ReactCardFlip>

               

                <ReactCardFlip  isFlipped={isFlipped2} flipDirection="horizontal">
                <div onClick={handleClick} > 
                <div   className="item">
                        <img id="2"  src="https://static.whatsapp.net/rsrc.php/v3/yO/r/FsWUqRoOsPu.png" alt="" />
                        <h3 id="2">Whatsapp Clone</h3>
                    </div>
                       
                    </div>

                    <div id="2" onClick={handleClick} className="backItem">
                         <h3 id="2" className='backTop'>Whatsapp Clone</h3>
                         <span id="2" className="backText">
                        In This Project i learned about websockets and socket.io and improved my React coding skills ( Importing technique, new Hooks , Folders arrangement )
                        </span>   
                        <div className="iconButton">
                        <IconButton>
                        <a href="https://github.com/deangal/WhatsappClone">
                        <GitHub/>
                        </a>
                        </IconButton> 
                        
                        </div>
                </div>
                </ReactCardFlip>

                <ReactCardFlip isFlipped={isFlipped3} flipDirection="horizontal">
                <div onClick={handleClick} > 
                <div className="item">
                        <img id="3" src="https://acquire.io/wp-content/uploads/2017/12/7-Ecommerce-Technology-Trends-that-Empower-Businesses-Updated.png" alt="" />
                        <h3 id="3">e-commerce App</h3>
                    </div>
                       
                    </div>

                    <div id="3" onClick={handleClick} className="backItem">
                         <h3 id="3" className='backTop'>e-commerce App</h3>
                         <span id="3" className="backText">
                        This project was more about approaching to the e-commerce market and learning how to implant payments via stripe
                         

                        </span>  
                        <div className="iconButton">
                        <IconButton>
                        <a href="https://github.com/deangal">
                        <GitHub/>
                        </a>                       
                        </IconButton> 
                       
                        </div>
                </div>
                </ReactCardFlip>

                <ReactCardFlip isFlipped={isFlipped1} flipDirection="horizontal">
                <div  onClick={handleClick} className="item">
                        <img id="1" src="https://www.eff.org/files/issues/student-privacy4.jpg" alt="" />
                        <h3 id="1">Basic html/api App</h3>
                    </div>
                       
                    

                    <div id="1" onClick={handleClick} className="backItem">
                         <h3 id="1" className='backTop'>Basic HTML/API App</h3>
                         <span id="1" className="backText">
                        In this project I learned on basic HTML and how a api works
                        This project was more about understanding what goes behind the scenes of React and HTTP requests
                        
                        </span>   
                        <div className="iconButton">
                        <IconButton>
                        <a href="https://github.com/deangal/HTMLAPI">
                        <GitHub/>
                        </a>                        </IconButton>
                        
                        </div>
                        
                </div>
                </ReactCardFlip>
                </div>

                
               
        </div>
    );
};

export default Protfolio;